...
st.set_page_config(
    page_title="Casa Marques Financeiro",
    page_icon="💼",
    layout="wide",
...
.main .block-container {padding-top:.7rem; padding-bottom:5rem; max-width:520px;}
...
.avatar{width:50px;height:50px;border-radius:50%;background:linear-gradient(145deg,#5B42FF,#8B5CF6);color:#fff;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:18px;box-shadow:0 12px 25px rgba(91,66,255,.28);}
...
.kpi-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:10px 0 16px;}
.kpi-card{min-height:122px;border-radius:24px;padding:18px;border:1px solid rgba(226,232,240,.9);box-shadow:0 14px 32px rgba(15,23,42,.08);position:relative;overflow:hidden;}
.kpi-green{background:linear-gradient(145deg,#E9FFF1,#FFFFFF);}
.kpi-red{background:linear-gradient(145deg,#FFF0F1,#FFFFFF);}
.kpi-blue{background:linear-gradient(145deg,#EEF2FF,#FFFFFF);}
.kpi-soft{background:linear-gradient(145deg,#F4F7FF,#FFFFFF);}
.icon-round{width:48px;height:48px;border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:25px;margin-bottom:12px;}
.icon-green{background:#CFFAD9}.icon-red{background:#FFE0E2}.icon-blue{background:#DDD8FF}.icon-soft{background:#E8EEFF}
.card-label{font-size:13px;font-weight:850;color:#26324A;margin-bottom:8px;}
.card-value{font-size:21px;font-weight:950;letter-spacing:-.03em;}
.green{color:#05A839}.red{color:#E11928}.blue{color:#183CE7}.purple{color:#5B42FF}

.model6-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin:18px 0;}
.btn3d{min-height:132px;border-radius:30px;padding:18px 14px;color:white;position:relative;overflow:hidden;box-shadow:0 20px 38px rgba(15,23,42,.18), inset 0 3px 10px rgba(255,255,255,.22);border:1px solid rgba(255,255,255,.26);}
.btn3d:before{content:"";position:absolute;top:-35%;right:-25%;width:110px;height:110px;background:rgba(255,255,255,.20);border-radius:50%;}
.btn3d-icon{font-size:40px;margin-bottom:12px;filter:drop-shadow(0 8px 14px rgba(0,0,0,.20));}
.btn3d-title{font-size:17px;font-weight:950;letter-spacing:-.02em;}
.btn3d-sub{font-size:12px;font-weight:700;opacity:.88;margin-top:4px;}
.btn-receita{background:linear-gradient(145deg,#22C55E,#049A39);}
.btn-despesa{background:linear-gradient(145deg,#FF4D5E,#D51020);}
.btn-relatorio{background:linear-gradient(145deg,#3B82F6,#1D4ED8);}
.btn-admin{background:linear-gradient(145deg,#7C3AED,#4F46E5);}
...
.tx-ico{width:46px;height:46px;border-radius:17px;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;}
.tx-title{font-weight:900;color:#111827;font-size:14px;}
.tx-sub{font-size:12px;color:#64748B;margin-top:3px;font-weight:600;}
.tx-val{font-weight:950;text-align:right;font-size:14px;}
.badge{display:inline-block;padding:4px 9px;border-radius:999px;font-size:11px;font-weight:850;margin-top:4px;}
.badge-green{background:#DCFCE7;color:#138A36}.badge-red{background:#FFE3E5;color:#E11928}.badge-amber{background:#FEF3C7;color:#A16207}

.stButton>button{width:100%;border-radius:20px;min-height:52px;font-weight:900;border:1px solid #E6E9F2;background:#fff;color:#17203A;box-shadow:0 8px 18px rgba(15,23,42,.06);}
...
.receita-btn{background:linear-gradient(145deg,#E8FBEF,#FFFFFF);border:1px solid #C7F3D5;border-radius:24px;padding:20px;text-align:center;font-size:18px;font-weight:950;color:#078A2D;box-shadow:0 10px 22px rgba(34,197,94,.14);}
.despesa-btn{background:linear-gradient(145deg,#FFF0F1,#FFFFFF);border:1px solid #FFD5D9;border-radius:24px;padding:20px;text-align:center;font-size:18px;font-weight:950;color:#D51020;box-shadow:0 10px 22px rgba(239,68,68,.14);}
...
def senha_hash(s):
    return hashlib.pbkdf2_hmac("sha256", str(s).encode(), b"casa_marques_modelo6", 140000).hex()
...
def empresas():
    con = db()
    df = pd.read_sql_query("SELECT id,nome FROM empresas WHERE status='Ativa' ORDER BY nome", con)
    con.close()
    return df
...
def autenticar(empresa, usuario, senha):
    ...
    row = cur.fetchone()
    con.close()
    if not row or not senha_ok(senha, row[6]):
        return None
...
def pergunta_rec(empresa,usuario):
    con = db()
    cur = con.cursor()
    cur.execute("SELECT u.pergunta FROM usuarios u JOIN empresas e ON e.id=u.empresa_id WHERE lower(e.nome)=lower(?) AND lower(u.usuario)=lower(?)", (empresa.strip(), usuario.strip().lower()))
    row = cur.fetchone()
    con.close()
...
def resetar(empresa,usuario,resposta,nova):
    con = db()
    cur = con.cursor()
    cur.execute("SELECT u.id,u.resposta_hash FROM usuarios u JOIN empresas e ON e.id=u.empresa_id WHERE lower(e.nome)=lower(?) AND lower(u.usuario)=lower(?)", (empresa.strip(), usuario.strip().lower()))
    row = cur.fetchone()
    if not row:
        con.close()
...
def carregar(emp_id):
    ...
    con = db()
    df = pd.read_sql_query("SELECT * FROM lancamentos WHERE empresa_id=? ORDER BY date(data_lancamento) DESC, id DESC", con, params=(emp_id,))
    con.close()
    for c in cols:
...
def excluir(auth, lanc_id):
    con = db()
    con.execute("DELETE FROM lancamentos WHERE id=? AND empresa_id=?", (lanc_id, auth["empresa_id"]))
    con.commit()
    con.close()
...
def filtro_mes(key):
    ...
    with c1:
        mes = st.selectbox("Mês", range(1,13), index=hoje.month-1, format_func=lambda x:f"{x:02d}", key=f"mes_{key}")
    with c2:
        ano = st.number_input("Ano", 2020, 2100, hoje.year, key=f"ano_{key}")
...
def login():
    ...
    with t2:
        ...
        if criar:
            ok,msg = criar_conta(empresa,nome,usuario,senha,pergunta,resposta)
    ...
    with t3:
        ...
        if "rec" in st.session_state:
            ...
            if alt:
                ok,msg = resetar(st.session_state["rec"]["empresa"],st.session_state["rec"]["usuario"],resposta,nova)
                st.success(msg) if ok else st.error(msg)
...
def kpi_cards(df):
    ...
    st.markdown(f"""
    <div class="kpi-grid">
      <div class="kpi-card kpi-green"><div class="icon-round icon-green">💼</div><div class="card-label">Receitas do mês</div><div class="card-value green">{brl(rec)}</div></div>
      <div class="kpi-card kpi-red"><div class="icon-round icon-red">💳</div><div class="card-label">Despesas do mês</div><div class="card-value red">{brl(desp)}</div></div>
      <div class="kpi-card kpi-blue"><div class="icon-round icon-blue">📈</div><div class="card-label">Saldo do mês</div><div class="card-value blue">{brl(saldo)}</div></div>
      <div class="kpi-card kpi-soft"><div class="icon-round icon-soft">📄</div><div class="card-label">Lançamentos</div><div class="card-value purple">{qtd}</div></div>
    </div>
    """, unsafe_allow_html=True)
...
def lista(df):
    ...
    for _,r in df.iterrows():
        ...
        badge = "badge-green" if r["tipo"]=="Receita" else "badge-red"
...
def admin(auth):
    st.markdown('<div class="topbar"><div><div class="top-title">Admin</div><div class="top-sub">Usuários e permissões</div></div></div>', unsafe_allow_html=True)
    if st.button("Sair"):
        st.session_state.clear()
        st.rerun()
    if auth["perfil"]!="Admin":
        st.info("Usuário sem permissão administrativa.")
        return
    con = db()
    users = pd.read_sql_query("SELECT id,usuario,nome,perfil,status FROM usuarios WHERE empresa_id=? ORDER BY usuario", con, params=(auth["empresa_id"],))
    con.close()
    for _,u in users.iterrows():
        b = "badge-green" if u["status"]=="Ativo" else "badge-red"
        st.markdown(f'<div class="user-row"><div class="user-ball">{u["usuario"][0].upper()}</div><div style="flex:1"><b>{u["nome"]}</b><br><span style="font-size:12px;color:#64748B;font-weight:600;">{u["perfil"]}</span></div><span class="badge {b}">{u["status"]}</span></div>', unsafe_allow_html=True)
    ...
    with t1:
        ...
        if criar:
            try:
                con=db()
                con.execute("INSERT INTO usuarios (empresa_id,usuario,nome,senha_hash,perfil,status,pergunta,resposta_hash,criado_em) VALUES (?,?,?,?,?,'Ativo','Código de recuperação',?,?)", (auth["empresa_id"],usuario.lower(),nome,senha_hash(senha),perfil,senha_hash(senha),datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    ...
    with t2:
        if not users.empty:
            op={f'{r.usuario} | {r.perfil} | {r.status}':int(r.id) for r in users.itertuples()}
            uid=op[st.selectbox("Usuário", list(op.keys()))]
            nova=st.text_input("Nova senha", type="password")
...
def nav_buttons(page):
    c = st.columns(5)
    items = [("Resumo","🏠 Resumo"),("Lançamentos","💬 Lanç."),("Novo","＋ Novo"),("Relatórios","📊 Relat."),("Admin","⚙️ Admin")]
    for col,(p,label) in zip(c,items):
        with col:
...
def app():
    ...
    if page=="Resumo":
        ...
        mes,ano=filtro_mes("resumo")
        dfm=filtrar(carregar(auth["empresa_id"]),mes,ano)
        kpi_cards(dfm)
        modelo6_buttons()
        st.markdown('<div class="section">Últimos lançamentos</div>', unsafe_allow_html=True)
        lista(dfm)
    elif page=="Lançamentos":
        st.markdown('<div class="topbar"><div><div class="top-title">Lançamentos</div></div></div>', unsafe_allow_html=True)
        mes,ano=filtro_mes("lanc")
        dfm=filtrar(carregar(auth["empresa_id"]),mes,ano)
        busca=st.text_input("Buscar lançamento...")
        if busca and not dfm.empty:
            dfm=dfm[dfm["descricao"].str.contains(busca,case=False,na=False)]
        lista(dfm)
    elif page=="Novo":
        st.markdown('<div class="topbar"><div><div class="top-title">Novo Lançamento</div></div></div>', unsafe_allow_html=True)
        ok,d=form_lanc(key="novo")
        if ok:
            if not d["descricao"] or d["valor"]<=0:
        ...
        st.markdown('<div class="topbar"><div><div class="top-title">Relatórios</div></div></div>', unsafe_allow_html=True)
        mes,ano=filtro_mes("rel")
        dfm=filtrar(carregar(auth["empresa_id"]),mes,ano)
        kpi_cards(dfm)
        if not dfm.empty:
            cat=dfm.groupby(["tipo","categoria"],as_index=False)["valor"].sum()
        ...
        with t2:
            ...
            if lid:
                ...
                if st.button("Atualizar"):
                    con=db()
                    con.execute("UPDATE lancamentos SET status=?, atualizado_em=? WHERE id=? AND empresa_id=?", (ns,datetime.now().strftime("%Y-%m-%d %H:%M:%S"),lid,auth["empresa_id"]))
                    con.commit(); con.close()
